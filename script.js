...
document.querySelector("#myform").addEventListener("submit", function () {
    ...
    emailjs.send("service_slgw7ij", "template_quxscbq", data).then(function (res) {
...
let selectdata = document.getElementById("dataselect");
let opt = "";
for (const items of countries) {
    opt += `<option value="${items.code}">${items.code} +${items.phone}</option>`
    selectdata.innerHTML = opt;
}

function selectNum() {
    let Val = document.getElementById("dataselect").value;
    let selectOpt = countries.find(country => country.code === Val);
    if (selectOpt) {
        document.getElementById("phone").value = `+${selectOpt.phone}`;
    }
    else {
